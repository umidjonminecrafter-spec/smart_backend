...
from tasks.models import TaskPermission, Board, Column, Item
...
class HasBoardPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if view.action == 'create':
            ...
            if not board_id and 'column_id' in request.data:
                col = Column.objects.filter(id=request.data['column_id']).first()
                if col:
                    board_id = col.board_id
            elif not board_id and 'column' in request.data:
                col = Column.objects.filter(id=request.data['column']).first()
                if col:
                    board_id = col.board_id
